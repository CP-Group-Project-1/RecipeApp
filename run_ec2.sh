...
if [ -z "$1" ]; then
  ...
  echo "Usage: ./run_ec2.sh <DOCKERHUB_USERNAME> <IMAGE_VERSION> <IMAGE_VERSION> <AWS_SECRET_NAME>"
  exit 1
fi
...
if [ -z "$3" ]; then
  ...
  echo "Usage: ./run_ec2.sh <DOCKERHUB_USERNAME> <IMAGE_VERSION> <AWS_SECRET_NAME> "
  exit 1
fi
